...
class Solution:
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        for str_item in strs:
            str_item_sorted = sorted(str_item)
            str_item_sorted = "".join(str_item_sorted)
            if str_item_sorted not in items_dict:
                items_dict[str_item_sorted] = [str_item]
            else:
                items_dict[str_item_sorted].append(str_item)
...
if __name__ == "__main__":
    # solve()
    solve = Solution()
    res = solve.groupAnagrams(["eat", "tea", "tan", "ate", "nat", "bat"])
    print(res)
