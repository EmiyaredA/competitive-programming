    ...
    def maxArea(self, height: List[int]) -> int:
        ...
        while l_index < r_index:
            if height[l_index] < height[r_index]:
                max_area = max(max_area, (r_index-l_index)*height[l_index])
                ori_l_index = l_index
                while l_index < r_index:
                    l_index += 1
                    if height[l_index] > height[ori_l_index]:
                        break
            else:
                max_area = max(max_area, (r_index-l_index)*height[r_index])
                ori_r_index = r_index
                while l_index < r_index:
                    r_index -= 1
                    if height[r_index] > height[ori_r_index]:
                        break      
...
if __name__ == "__main__":
    # solve()
    solve = Solution()
    res = solve.maxArea([1,8,6,2,5,4,8,3,7])
    print(res)
