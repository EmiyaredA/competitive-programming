...
class Solution:
    def prefix_sum(self,arr): return list(accumulate(arr, initial=0)) # 前缀和数组
    
    def binary_search_check(self, lo, hi, arr, val):
        def check(index):
            if arr[index] < val:
                if index < len(arr) - 1 and arr[index+1] >= val:
                    return index + 1
    ...
    def subarraySum(self, nums: List[int], k: int) -> int:
        ...
        prefix_arr = self.prefix_sum(nums)
        prefix_dict = {}
        for index, sum in enumerate(prefix_arr):
            if str(sum)  not in prefix_dict:
                prefix_dict[str(sum)] = [index]
            else:
                prefix_dict[str(sum)].append(index)

        index = 1
        res = 0
        while index <= len(nums):
            prefix_select = prefix_dict.get(str(prefix_arr[index]-k))
            if prefix_select is not None:
                res += self.binary_search_check(0, len(prefix_select), prefix_select, index)
