...
class Solution:
    ...
    def lengthOfLongestSubstring(self, s: str) -> int:
        ...
        for index, c in enumerate(s):
            if c in char_set:
                curr_len = index - char_set[c] - 1
                tmp_st_index = char_set[c] + 1
                for c_index in range(st_index, char_set[c] + 1):
                    del char_set[s[c_index]]
                    del ordered_char_list[0]
                st_index = tmp_st_index
                         
            char_set[c] = index
            ordered_char_list.append(c)
            curr_len += 1  
...
if __name__ == "__main__":
    # solve()
    solve = Solution()
    res = solve.lengthOfLongestSubstring("abcabcbb")
    print(res)
